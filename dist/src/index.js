...
var getAccessorsOf = function (cl) {
    if (!cl)
        return [];
    return cl.getGetAccessors().concat(getAccessorsOf(cl.getBaseClass()));
};
var getMethodsOf = function (cl) {
    if (!cl)
        return [];
    return cl.getMethods().concat(getMethodsOf(cl.getBaseClass()));
};
var getPropertiesOf = function (cl) {
    if (!cl)
        return [];
    return cl.getProperties().concat(getPropertiesOf(cl.getBaseClass()));
};
                    ...
                    project.getSourceFiles().forEach(function (sourceFile) {
                        ...
                        sourceFile.getClasses().forEach(function (c) {
                            if (c
                                ...
                                var reducerMethods_1 = getMethodsOf(c).filter(function (m) {
                                    if (m.compilerNode.modifiers) {
                                        var isProtected_1 = false;
                                        m.compilerNode.modifiers.forEach(function (mm) {
                                            if (mm.getText().indexOf("protected") >= 0)
                                                isProtected_1 = true;
                                        });
                                        if (isProtected_1)
                                            return false;
                                ...
                                var selectorMethods_1 = getAccessorsOf(c).filter(function (m) {
                                ...
                                ng_1.indent(1);
                                getPropertiesOf(c).forEach(function (p) {
                                ...
                                // Create model of all the variables...
                                ng_1.raw("\n/**\n * @generated true\n */", true);
                                ng_1.out("export class R" + c.getName() + (" extends " + c.getName() + " {"), true);
                                ng_1.indent(1);
                                ...
                                selectorMethods_1.forEach(function (m) {
                                    ...
                                    if (rvNode) {
                                        ...
                                        var properties_1 = {};
                                        var methods_1 = {};
                                        getMethodsOf(c).forEach(function (m) {
