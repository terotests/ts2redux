...
const getAccessorsOf = (
  cl?: ClassDeclaration
): (MethodDeclaration | GetAccessorDeclaration)[] => {
  if (!cl) return [];
  return [...cl.getGetAccessors(), ...getAccessorsOf(cl.getBaseClass())];
};
const getMethodsOf = (cl?: ClassDeclaration): MethodDeclaration[] => {
  if (!cl) return [];
  return [...cl.getMethods(), ...getMethodsOf(cl.getBaseClass())];
};
const getPropertiesOf = (cl?: ClassDeclaration): PropertyDeclaration[] => {
  if (!cl) return [];
  return [...cl.getProperties(), ...getPropertiesOf(cl.getBaseClass())];
};
  ...
  project.getSourceFiles().forEach(sourceFile => {
    ...
    sourceFile.getClasses().forEach(c => {
      if (
        ...
        const reducerMethods = getMethodsOf(c).filter(m => {
          if (m.compilerNode.modifiers) {
            let isProtected = false;
            m.compilerNode.modifiers.forEach(mm => {
              if (mm.getText().indexOf("protected") >= 0) isProtected = true;
            });
            if (isProtected) return false;
          }
          return typeof m.getReturnTypeNode() === "undefined";
        });
        const selectorMethods = getAccessorsOf(c).filter(m => {
 ...
 */`,
        ...
        ng.out(
          "export class R" + c.getName() + ` extends ${c.getName()} {`,
          true
        );
        ng.indent(1);
        const body = ng.fork();
        ...
        getMethodsOf(c).forEach(m => {
          ...
          if (m.compilerNode.modifiers) {
            m.compilerNode.modifiers.forEach(mm => {
              if (mm.getText().indexOf("protected") >= 0) rvMethod = true;
            });
          }
          if (rvNode) {
            rvMethod = true;
